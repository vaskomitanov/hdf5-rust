...
use hdf5_sys::h5t::{H5Tget_class, H5T_ENUM, H5T_FLOAT, H5T_INTEGER};
use hdf5_sys::h5z::{
...
pub(crate) unsafe fn validate_filters(filters: &[Filter], datatype_id: hid_t) -> Result<()> {
    const COMP_FILTER_IDS: &[H5Z_filter_t] = &[H5Z_FILTER_DEFLATE, H5Z_FILTER_SZIP, 32000, 32001];

    let type_class = H5Tget_class(datatype_id);
